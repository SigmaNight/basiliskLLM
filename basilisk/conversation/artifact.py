...
class Artifact(BaseModel):
	...
	language: Optional[str] = Field(
		default=None, description="Programming language for code artifacts"
	)
	created_at: datetime = Field(default_factory=datetime.now)
	source_message_id: Optional[str] = Field(
		default=None, description="ID of source message"
	)
...
class ArtifactDetector:
	...
	def detect_artifacts(cls, content: str) -> List[ArtifactMatch]:
		...
		for match in cls.TEXT_BLOCK_PATTERN.finditer(content):
			...
			if len(text_content) < 100 or any(
				start_pos < end and end_pos > start
...
class ArtifactManager:
	...
	def add_artifacts_from_content(
		self, content: str, source_message_id: Optional[str] = None
	) -> List[Artifact]:
