...
from basilisk.conversation.artifact import Artifact, ArtifactManager, ArtifactType
...
class ArtifactListCtrl(wx.ListCtrl):
	...
	def update_artifacts(self, artifacts: List[Artifact]) -> None:
		...
		for i, artifact in enumerate(artifacts):
			...
			self.SetItem(index, 1, artifact.type.value.replace('_', ' ').title())
...
class ArtifactDialog(wx.Dialog):
	...
	def _bind_events(self) -> None:
		...
		self.artifact_list.Bind(wx.EVT_LIST_ITEM_SELECTED, self._on_selection_changed)
		self.artifact_list.Bind(wx.EVT_LIST_ITEM_DESELECTED, self._on_selection_changed)
	...
	def _on_view(self, event: wx.Event) -> None:
		...
		if artifact.type == ArtifactType.CODE_BLOCK:
			...
			format_type = "markdown" if artifact.type == ArtifactType.MARKDOWN else "html"
...
def show_artifact_dialog(parent: wx.Window, conversation_tab: 'ConversationTab') -> None:
