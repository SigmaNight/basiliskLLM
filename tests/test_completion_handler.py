...
@pytest.fixture
def block_with_think_tags(ai_model):
	"""MessageBlock with leading </think>...</think> block in content (legacy combined)."""
	req = Message(role=MessageRoleEnum.USER, content="Think")
	resp = Message(
		role=MessageRoleEnum.ASSISTANT,
		content=f"{START_BLOCK_REASONING}\ninternal reasoning\n{END_REASONING}\n\nfinal answer",
		reasoning=None,
	)
	return MessageBlock(request=req, response=resp, model=ai_model)
...
class TestSplitReasoningFromContent:
	...
	def test_parses_think_tags_into_reasoning_and_content(
		self, handler, block_with_think_tags
	):
		"""Leading </think>...</think> block is split into reasoning and content."""
		handler._split_reasoning_from_content(block_with_think_tags)
		assert block_with_think_tags.response.reasoning == "internal reasoning"
		assert block_with_think_tags.response.content == "final answer"
