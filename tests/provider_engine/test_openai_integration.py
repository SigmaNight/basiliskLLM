...
class TestResponsesAPIIntegration:
	...
	def test_responses_api_input_preparation(
	...
	def test_responses_api_completion_flow(
		self, engine, conversation_with_history
	):
	...
	def test_chat_api_fallback_flow(self, engine, conversation_with_history):
		...
		with patch.object(engine, 'client') as mock_client:
			...
			result = engine.completion(
				chat_block, conversation_with_history, None
			)
	...
	@pytest.mark.parametrize(
		...
		[
			...
			(
				"response.completed",
				{"response": {"output_text": "Complete"}},
				["Complete"],
			),
		],
	)
	def test_streaming_event_handling(
		self, engine, chunk_type, chunk_data, expected
	):
		...
		if chunk_type == "response.output_text.delta":
			...
			mock_chunk.item.content[0].text = chunk_data["item"]["content"][0][
				"text"
			]
		elif chunk_type == "response.completed":
			mock_chunk.response = Mock()
			mock_chunk.response.output_text = chunk_data["response"][
				"output_text"
			]
	...
	def test_reasoning_model_parameters(self, engine):
		...
		new_block = MessageBlock(
			request=Message(
				role=MessageRoleEnum.USER, content="Test reasoning"
			),
			model=AIModelInfo(
